...
import { Boxes, Loader2, Pencil, Search, Trash2 } from "lucide-react";
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    async function loadInventory() {
      try {
        const res = await fetch("/api/items?pageSize=100", { cache: "no-store" });
        if (!res.ok) throw new Error("Failed to fetch inventory data");
        const json = await res.json();
        setData(json.items ?? []);
      } catch (err) {
        console.error(err);
        setError("Gagal memuat data inventori. Coba lagi dalam beberapa saat.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const columns = useMemo<ColumnDef<InventoryItem>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => row.getValue<string | null>("brandModel") ?? "-",
      },
      {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-[#CFE6D6] bg-white p-6 shadow-sm">
              ...
              {loading && (
                <tr>
                  <td colSpan={table.getVisibleLeafColumns().length} className="px-4 py-10 text-center text-sm text-[#3E4643]">
                    <Loader2 className="mx-auto h-5 w-5 animate-spin text-[#216B5B]" />
                    <span className="mt-2 block">Memuat data inventori...</span>
                  </td>
                </tr>
              )}
        ...
        {error && (
          <p className="rounded-2xl border border-[#F6B8B8] bg-[#F6B8B8]/30 px-4 py-3 text-sm text-[#A83232]">
            {error}
          </p>
        )}
      </div>
